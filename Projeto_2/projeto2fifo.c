...
void FIFO(TLB *table, int page_number, int frame_number);
...
int main(int argc, char *argv[]){
  ...
  for(int i = 0; i < number_of_addresses; i++) {
    ...
    if(flag == 1) {
      ...
      FIFO(table, table[found].page_numb, table[found].frame_numb);
      ...
      FIFO(table, page_position, TP[page_position]);
      printf("Número de Página: %d\n", page_position);
      printf("O endereço lógico que está sendo traduzido: %d\n", (*(logical + i)).pure_numb);
      printf("O valor do offset: %d\n", (*(logical + i)).disp_numb);
      printf("Endereço na memória física: %d\n", TP[page_position]);
  ...
  printf("O número de acessos pela TLB: %d\nA técnica de substituição de página utilizada foi FIFO\n", tlb_sucess);
